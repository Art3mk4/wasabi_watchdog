...
export default class ScraperJob extends BaseJob {
  ...
  async parse() {
    ...
    try {
      ...
      const items: ItemData[] = await page.evaluate(() => {
        return [...Object.values(document.querySelectorAll('._h3pmwn')).map((item) => {
          return {
            name: item.parentElement.parentElement.parentElement.querySelector('._16s5yj36').childNodes[0].data || '',
            text:  item.childNodes[0].data || '',
            date: item.parentElement.parentElement.parentElement.querySelector('._139ll30').childNodes[0].data || '',
            rating: item.parentElement.parentElement.parentElement.querySelector('._1fkin5c').childNodes.length || null
          }
  ...
  createDateObject(dateString: string) {
    ...
    return new Date(Number(year), this.getMonthIndex(month), parseInt(day, 10));
  }
